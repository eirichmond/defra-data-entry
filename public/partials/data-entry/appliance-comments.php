...
<table class="table table-bordered table-striped mb-4">
    ...
    <tbody>
        <?php foreach ($comments as $comment) { $userdata = get_user_by( 'ID', $comment->user_id ); ?>
            <tr>
                <td><?php echo esc_html( $public_class->get_comment_type_by_key(get_comment_meta($comment->comment_ID, 'comment_type_id', true )) ); ?></td>
                <td><?php echo esc_html( $public_class->get_comment_action_by_key(get_comment_meta($comment->comment_ID, 'comment_action_id', true )) ); ?></td>
